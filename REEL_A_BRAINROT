...
local SHOP_CONFIG = {
    ...
    { key="shop_rod",     label="Kraken's Rod", on=false,
      check = function(n) return n == "krakensrod" end },
}
...
local function comprarViaMerchantRemote()
    ...
    local rfBuy      = rh:FindFirstChild("KrakenMerchantBuy")
    local rfSnapshot = rh:FindFirstChild("KrakenMerchantSnapshot")
    ...
    if not snap.offers or not snap.currency then return end

    local monedas = snap.currency.amount or snap.currency
    print("[AutoShop Kraken] Monedas: " .. monedas)
    ...
    for i, offer in ipairs(snap.offers) do
        ...
        local purchased = offer.purchased
        local name = tostring(id or ""):lower()

        if not itemDeseado(name) then continue end

        if stock and stock > 0 and not purchased and monedas >= cost then
            print("[AutoShop Kraken] Comprando: " .. name)
            local ok1, r1 = pcall(function() return rfBuy:InvokeServer(i) end)
            if ok1 and r1 then
                monedas = monedas - cost
                print("[AutoShop Kraken] OK: " .. name)
            else
                local ok2, r2 = pcall(function() return rfBuy:InvokeServer(id) end)
                if ok2 and r2 then
                    monedas = monedas - cost
                    print("[AutoShop Kraken] OK (id): " .. name)
                else
                    print("[AutoShop Kraken] Fallo: " .. name)
                end
            end
            task.wait(0.5)
        end
...
-- Posiciones exactas de cada slot (Kraken Event)
local SLOT_POS = {
    Vector3.new(-144.004, 2, -259.281),  -- Brainrot1
    Vector3.new(-136.004, 2, -259.281),  -- Brainrot2
    Vector3.new(-128.004, 2, -259.281),  -- Brainrot3
}
...
local function getSlots()
    ...
    local rfSnap = rh:FindFirstChild("KrakenSacrificeSnapshot")
    ...
    return snap.slots or snap
end

local function slotNecesita(slots, i)
    if not slots then return false end
    local slot = slots[i] or slots[tostring(i)]
    if slot then
        return slot.submitted == false or slot.occupied == false
    end
    return false
end
...
local function submitSlot(slotIndex, brainrotId)
    local rh = ReplicatedStorage:FindFirstChild("RemoteHandler")
    if not rh then return false end
    local submitRemote = rh:FindFirstChild("KrakenSacrificeSubmit")
    if submitRemote then
        local ok = pcall(function()
            submitRemote:InvokeServer(slotIndex, brainrotId)
        end)
        if ok then return true end
    end

    -- Fallback: teleport + ProximityPrompt igual que antes
    local c = LP.Character
    if not c then return false end
    ...
    local stand = Workspace:FindFirstChild("KrakenStand") or Workspace:FindFirstChild("StPatricksStand")
    if stand then
...
local function hacerClaim()
    local rh = ReplicatedStorage:FindFirstChild("RemoteHandler")
    if rh then
        local fn = rh:FindFirstChild("KrakenClaimFinal")
...
-- Detectar reset leyendo el timer del snapshot Kraken
local function getResetSegundos()
    local rh = ReplicatedStorage:FindFirstChild("RemoteHandler")
    if not rh then return nil end
    -- Intentar KrakenCounterSnapshot primero
    local rfCounter = rh:FindFirstChild("KrakenCounterSnapshot")
    if rfCounter then
        local ok, snap = pcall(function() return rfCounter:InvokeServer() end)
        if ok and snap then
            local t = snap.timeUntilReset or snap.resetIn or snap.timer or snap.timeLeft
            if t and t > 0 then return t end
        end
    end
    -- Fallback: KrakenSacrificeSnapshot
    local rfSnap = rh:FindFirstChild("KrakenSacrificeSnapshot")
    if rfSnap then
        local ok, snap = pcall(function() return rfSnap:InvokeServer() end)
        if ok and snap then
...
local AutoScriptLeft = AutoScriptTab:DrawSection({ Name = "Auto Farm", Position = "left", Minimized = true })
...
local AutoScriptRight = AutoScriptTab:DrawSection({ Name = "Upgrades", Position = "right", Minimized = true })
...
-- Dupe
local _Remote = game:GetService("ReplicatedStorage").RemoteHandler.Plot

local dupeStates = {
    AutoBase = false,
    Base1=false, Base2=false, Base3=false,
    Base4=false, Base5=false, Base6=false,
}
local dupeLoops = {}

local function _getDupeBrainrot()
    local charFolder = workspace:FindFirstChild(LP.Name)
    if charFolder then
        for _, v in pairs(charFolder:GetDescendants()) do
            if string.find(v.Name, "Brainrot") then return v.Name end
        end
    end
    local bp = LP:FindFirstChildOfClass("Backpack")
    if bp then
        for _, t in pairs(bp:GetChildren()) do
            if string.find(t.Name, "Brainrot") then return t.Name end
        end
    end
    return "Brainrot1:Normal:1:1:0"
end

local function _hasBrainrot(plotFolder)
    for _, v in pairs(plotFolder:GetChildren()) do
        if string.find(v.Name, "Brainrot") then return true end
    end
    return false
end

local function _getMyBase()
    ...
    for _, base in pairs(Bases:GetChildren()) do
        ...
        if ownerBoard then
            for _, desc in pairs(ownerBoard:GetDescendants()) do
                if desc:IsA("TextLabel") and (desc.Text == myName or desc.Text == myId) then
                    return objects
                end
            end
        end
        for _, child in pairs(base:GetChildren()) do
            if (child:IsA("StringValue") or child:IsA("IntValue")) then
                ...
                if val == myId or val == myName then return objects end
            end
        end
        local attrOwner = base:GetAttribute("Owner") or base:GetAttribute("OwnerId")
        if attrOwner and (tostring(attrOwner) == myId or tostring(attrOwner) == myName) then
            return objects
        end
    end
    return nil
...
local function _startDupe(key, getBaseFn)
    if dupeLoops[key] then return end
    dupeLoops[key] = task.spawn(function()
        local ITEM = _getDupeBrainrot()
        local Base = getBaseFn()
        if not Base then
            warn("[DUPE] Base no encontrada: " .. key)
            dupeStates[key] = false
            dupeLoops[key]  = nil
            return
        end
        print("[DUPE] " .. key .. " → " .. ITEM)
        ...
        for i = 1, 30 do
            threads[i] = task.spawn(function()
                while dupeStates[key] do
                    local p = "Plot" .. i
                    local f = Base:FindFirstChild(p)
                    if f and not _hasBrainrot(f) then
                        pcall(function() _Remote:FireServer("Add", p, ITEM) end)
                    end
                    task.wait(0.05)
                end
            end)
        end
        while dupeStates[key] do task.wait(0.5) end
        for _, t in pairs(threads) do pcall(task.cancel, t) end
        dupeLoops[key] = nil
    end)
end

local function _stopDupe(key)
    dupeStates[key] = false
    if dupeLoops[key] then task.cancel(dupeLoops[key]); dupeLoops[key] = nil end
end
...
local DupeLeft  = DupeTab:DrawSection({ Name = "Auto Base",   Position = "left",  Minimized = true })
local DupeRight = DupeTab:DrawSection({ Name = "Bases Fijas", Position = "right", Minimized = true })
...
DupeLeft:AddToggle({
    ...
    Default = false,
    Callback = function(v)
        dupeStates.AutoBase = v
        if v then _startDupe("AutoBase", _getMyBase) else _stopDupe("AutoBase") end
    end,
})

for _, baseName in ipairs({ "Base1","Base2","Base3","Base4","Base5","Base6" }) do
    local key = baseName
    DupeRight:AddToggle({
        Name    = key,
        Flag    = "dupe_" .. key:lower(),
        Default = false,
        Callback = function(v)
            dupeStates[key] = v
            if v then
                _startDupe(key, function()
                    local b = workspace.Bases:FindFirstChild(key)
                    return b and b:FindFirstChild("Objects") or nil
                end)
            else
                _stopDupe(key)
            end
